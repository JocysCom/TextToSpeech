...
<li>"WoW Addon" converts text message to line of coloured pixels and shows this line inside your in-game window. Default line's position is Top(0) Left(0):<br />
...
<li>"TTS Monitor" will capture this line of coloured pixels and convert it back to the text message if "Monitor: Display" capturin mode is enabled. Then, "TTS Monitor", <em>which must be running in the background</em>, converts it to SAPI XML and reads it with specific text-to-speech voice. Addon also supports option which allows to use manually pre-recorded voices.</li>
</ol>
