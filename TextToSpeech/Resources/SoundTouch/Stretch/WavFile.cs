...
namespace SoundStretch
{
	...
	public class WavInFile : IDisposable
	{
		...
		/// </summary>
		public static int Read(Stream stream, out string value, int count)
		{
		...
		private int ReadHeaderBlock()
		{
			// lead label string
			string label;
			if (Read(_fileStream, out label, 4) != 4) return -1;
			...
			if (label == WavFormat.FMT_STR)
			{
				...
				int nLen;
				if (Read(_fileStream, out nLen) != sizeof(int)) return -1;
			...
			if (label == WavFact.FACT_STR)
			{
				...
				// read data
				if (Read(_fileStream, out _header.Fact.FactSampleLength) != nLen) return -1;
			...
			if (label == WavData.DATA_STR)
			{
				...
				if (Read(_fileStream, out _header.Data.DataLen) != sizeof(int)) return -1;
			...
			// read length
			int len;
			if (Read(_fileStream, out len) != sizeof(int)) return -1;
