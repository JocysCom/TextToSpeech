...
namespace JocysCom.ClassLibrary.Runtime
{
    public class Serializer
    {
        ...
        {
            return (T)Serializer.DeserializeFromBytes(bytes, typeof(T));
        }
        ...
        static XmlSerializer GetXmlSerializer(Type type)
        {
            lock (XmlSerializersLock)
            {
                ...
                if (!XmlSerializers.ContainsKey(type))
                {
                    ...
                    extraTypes.Add(typeof(string));
                    extraTypes.AddRange(XmlSerializerExtraTypes);
                    XmlSerializers.Add(type, new XmlSerializer(type, extraTypes.ToArray()));
                }
            }
            return XmlSerializers[type];
        }

        public static List<Type> XmlSerializerExtraTypes = new List<Type>();
